...
def name_check():
    while True:
        name_input = info_input('Student name:\t')
        if name_input in student_data:
            print('WARNING\tStudent already exists in system.')
            yes_no = info_input('Continue?\t')
            if yes_no not in yn_list:
...
def new_student(): #gathering data to write into file
    ...
    new_name = name_check()#info_input('Student name:\t')
        ...
        if name_input not in student_data:
            ...
            if len(test_format) > 2: #if there are more than 2 tests in the system
                ...
                for brackets in test_format[2:]:
                    ...
                    modified = str(display + add_bracket)
                    modified_display = modified.format(lookup_user[z])
                    y = y + 1
                    z = z + 1         
